...
namespace VacationRental.Api.Tests
{
    ...
    public class GetCalendarTests
    {
        ...
        [Fact]
        public async Task GivenCompleteRequest_WhenGetCalendar_ThenReturnsCalendarWithPreparationTimes()
        {
            var postRentalRequest = new RentalBindingModel
            {
                Units = 2,
            ...
            using (var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={postRentalResult.Id}&start=2000-01-01&nights=4"))
            {
                Assert.True(getCalendarResponse.IsSuccessStatusCode);
                ...
                Assert.Contains(getCalendarResult.Dates[2].Bookings, x => x.Id == postBookingResult.Id);

                Assert.Single(getCalendarResult.Dates[3].PreparationTimes);
                Assert.Contains(getCalendarResult.Dates[3].PreparationTimes, x => x.Unit == postBookingResult.Id);
            }
        }
    }
}
